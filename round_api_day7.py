...
dt = str(api_date + timedelta(days=7))

# S3 연결 정보
s3_bucket = 'de-6-2-bucket'
s3_key = 'round_trip/'+dag_date+'_day7'+'.csv' # 현재 날짜에서 5일 뒤로 수정
...
# Airflow DAG에 태스크 추가
with DAG('round_api_day7', default_args=default_args, description='Fetch flight offers from Amadeus API and save to S3', schedule_interval=timedelta(days=1), catchup = False) as dag:
